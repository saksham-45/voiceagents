...
QList<IntentProfile> defaultIntentProfiles()
{
	return
	{
		...
		{ QStringLiteral( "pattern.rhythm" ), QStringLiteral( "safe" ), QStringLiteral( "PATTERN_RHYTHM" ), { QStringLiteral( "addkick" ), QStringLiteral( "addsnare" ), QStringLiteral( "add808" ) } },
	...
	const QString& riskHint )
{
	...
	if( needsConfirmation )
	{
		const int confirmWindowMs = qEnvironmentVariableIntValue( "LMMS_CONFIRM_WINDOW_MS" ) > 0
			...
			: 9000;
		m_pendingConfirmationCommand = commandText;
